...
- [x] Request and receive a separate explicit immediate approval before the `deployment_versel` push that triggers Production; granted on 2026-08-23.

## Authorized Production Release of Public Motion Prototype

- [x] Confirm the release source (`main`) and protected destination (`deployment_versel`) are clean, current, and safe to merge; source `ff7182a`, destination `7ebeb84` verified on 2026-08-23.
...
- [x] Create a non-Production reconciliation branch from `deployment_versel` and inventory deployment-protected files: Vercel routing, API bridge, server runtime, migrations, environment handling, and deployment configuration; local branch `release/ui-sync-deployment` starts at `7ebeb84`.
