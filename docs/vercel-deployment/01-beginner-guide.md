...
| **Preview** | A live but non-production test URL. | Vercel deploys `deployment_versel` here so `/`, `/edit`, database persistence, and uploads can be tested without a Production promotion. |
| **Production** | The site visitors reach through the Production domain. | Do not change it unless the user explicitly approves the specific release. |
...
> For this portfolio, `main` means **stable development** and `deployment_versel` means **Vercel deployment candidate**. The names describe their roles; they do not make a Production action safe by themselves.
...
3. The reviewed commits move to `deployment_versel`.
4. Vercel builds a Preview deployment from that branch.
5. The developer tests the Preview URL, especially `/`, `/edit`, and any affected database or file-upload workflow.
6. Only after an explicit decision can the user decide whether Production should receive that release.
