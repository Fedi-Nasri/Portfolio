...
| I-008 | Active | High | On 2026-08-22, Vercel Production Branch Tracking was explicitly changed from `main` to `deployment_versel`. Future pushes to `deployment_versel` create Production Deployments; no deployment was triggered by saving the setting. | Treat every push to `deployment_versel` as a Production release. Require explicit user release approval, completed validation, and post-deploy verification. |
...
| I-011 | Active | High | The approved 2026-08-23 `main` → `deployment_versel` release was stopped before any push because `git merge-tree --write-tree` reported unrelated histories. The raw 90-file difference treats Vercel API/runtime and migration files as deletions from `main`. | Do not use `--allow-unrelated-histories`, force-push, reset, or manual file copying. Prepare a reconciled, validated release candidate that retains Production infrastructure, then obtain renewed immediate release approval before pushing. |
