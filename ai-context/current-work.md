...
**Motion and Home-composition prototype branch:** `feature/public-motion-prototype` was created from `main` locally and on GitHub. Its latest reviewed commit is `0b5401d` (`feat: redesign editable home hero composition`). It adds a staged hero entrance and one-time IntersectionObserver section reveals, scoped under `.public-motion` so `/edit` is unchanged. It now also adapts the supplied wide three-column Home reference: Fedi’s existing editable identity panel remains left, his portrait remains centred, and the four existing editable focus areas form a right-side two-by-two card system. Editor mode retains in-place text edits, portrait/focus-visual uploads, focus-card dragging, and Reset card layout. The work honors `prefers-reduced-motion`, uses an observer fallback that leaves content visible, passes 81 tests plus the production build, and awaits user review before any merge.
