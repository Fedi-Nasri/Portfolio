...
**Latest saved project checkpoint:** `5b22d2d7` — captured the validated isolated Project image-panel discoverability repair. It introduces no Production, Vercel, database, Blob, domain, or secret change.
**Current immediate task:** The latest Production release is `deployment_versel` commit `70a4e3f`. The user requested a new isolated feature branch from that baseline: `feature/toolbox-devops-cloud`. It includes the role-focused, text-only Toolbox refinement and Selected Work enhancements. The Toolbox now provides restrained hover feedback and editor keyboard-focus feedback for skill chips, while preserving reduced-motion behavior. Its role groups place OVHcloud with Cloud Engineering; Wireshark, 802.1X Authentication, and Firewalls with Network Operations; and PostgreSQL, MySQL, Redis, and Firebase with Site Reliability. Every project has a migration-safe editable `summary`; existing saved projects derive a one-sentence summary from their body only when no summary is stored. Collapsed public and editor project cards hide images and use a two-column desktop layout: identity plus brief description on the left, and Tech stack plus Delivery metadata on the right. On mobile the metadata stacks below the summary. The accessible “View detailed project” disclosure retains the existing original image, case-study blocks, Tech stack, Delivery, and optional links. In `/edit`, the fixed right-side **Project image** panel now has an always-visible Add image or Replace image action for the selected project, a conditional Remove image action, explicit image status, and disabled crop/focal/ratio/frame controls when no image exists. Sidebar project selection synchronizes with the active editor project; removal clears saved focal point, zoom, aspect ratio, frame height, and any open focal picker. TypeScript, all 15 test files / 87 tests, and production builds passed. Production remains unchanged while this feature awaits review. The historical-media migration remains paused; its legacy references stay untouched in private Draft 2 until the user explicitly resumes it.
...
|---|---|---|
| `pnpm check` | Passed | TypeScript compiled with no errors after the Project image-panel repair. |
| `pnpm test` | Passed: 15 files, 87 tests | Includes Project image-panel add/replace/remove visibility coverage plus PostgreSQL persistence, Blob validation, API bridge, media upload preparation, public motion, Toolbox migration/editor parity, text formatting, and development-only pg-mem fallback coverage. |
| `pnpm build` | Passed | Vite output and server bundle built successfully after the Project image-panel repair; bundle-size warning is informational. |
