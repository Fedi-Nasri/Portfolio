...
**Motion and Home-composition prototype branch:** `feature/public-motion-prototype` was created from `main` locally and on GitHub. It adds a staged hero entrance and one-time IntersectionObserver section reveals, scoped under `.public-motion` so `/edit` is unchanged. The branch now uses the user-approved **featured specialty lens plus numbered rail** instead of equal role cards: one selected editable Cloud/DevOps/DevSecOps/Security & Networking visual appears as a circular pale-blue lens, while a compact text-first rail selects all four roles. The interaction works by click and standard keyboard button activation, the selected-lens transition is disabled for reduced-motion preferences, and mobile collapses into one lens plus a two-column rail. Editor mode retains in-place text edits, portrait/focus-visual uploads, focus-card drag/reset data, and its selected-text Bold action. The work passes 83 tests plus the production build and awaits user review before any merge.
