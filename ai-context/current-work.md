...
**Current immediate task:** Add source-defined Selected Work case studies for **1111.tn DevSecOps Delivery** and **Cloud & Kubernetes Modernization** (Independent Consulting), and promote Freelance Cloud & Kubernetes Engineer to the sole current/latest Experience entry. The 1111.tn case study covers environment promotion, GitHub Actions/Docker, SonarQube, Trivy, OWASP ZAP, Prometheus, and Grafana. The freelance case study covers Kubernetes production upgrades, on-premises-to-Azure migration, cloud-service implementation, and application deployment. The project migration adds these entries and the six-system heading only to the exact four-project default baseline, leaving saved custom project content unchanged. Source validation passed TypeScript, 17 Vitest files / 94 tests, and production build. The change is local-only and requires a fresh explicit approval before pushing `master`.
