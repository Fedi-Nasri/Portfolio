...
**Current immediate task:** Repair the just-released Selected Work migration so the saved Main draft receives the approved 1111.tn and Freelance case studies. The first release `a217de6` was Ready, but the persisted legacy projects lacked `summary` fields because those were previously hydrated in memory rather than saved. The narrow repair accepts a missing legacy summary while still rejecting a non-empty custom summary or any other project customization. It preserves all existing custom project data, adds the two requested case studies and six-system heading only to the exact legacy baseline, and has passed TypeScript, 17 Vitest files / 94 tests, and production build. It is local-only and needs a fresh explicit Production approval before pushing `master`.
