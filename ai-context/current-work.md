...
**Motion and Home-composition prototype branch:** `feature/public-motion-prototype` was created from `main` locally and on GitHub. It adds a staged hero entrance and one-time IntersectionObserver section reveals, scoped under `.public-motion` so `/edit` is unchanged. The current revision replaces all Home specialty SVG/image panels with four **text-only, editable floating labels**—Cloud, DevOps, DevSecOps, and Security & Networking—arranged in a balanced orbit beside the portrait. A low-contrast pale-blue dot grid, orbital outlines, and soft glow create the requested background theme without reducing readability. The labels stack into a clean two-column mobile layout; the subtle hover motion is disabled for reduced-motion preferences. Focus-visual and drag/reset data remain preserved but are intentionally not shown in this composition. Editor mode retains in-place label editing, portrait upload, and selected-text Bold action. The work passes 82 tests plus the production build and awaits user review before any merge.
