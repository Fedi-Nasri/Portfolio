...
**Latest saved project checkpoint:** `205244d0` — refined the isolated text-only specialty design with unnumbered labels, calm floating motion, and CSS-only four-discipline background motifs. No Production behavior changed.
...
**Motion and Home-composition prototype branch:** `feature/public-motion-prototype` was created from `main` locally and on GitHub. It adds a staged hero entrance and one-time IntersectionObserver section reveals, scoped under `.public-motion` so `/edit` is unchanged. The current revision replaces all Home specialty SVG/image panels with four **text-only, editable floating labels**—Cloud, DevOps, DevSecOps, and Security & Networking—arranged in a balanced orbit beside the portrait. Numeric prefixes are removed. Each label now has a low-amplitude staggered float (3–5px over 7–9 seconds) that is disabled for reduced-motion preferences. A low-contrast pale-blue dot grid, orbital outlines, and four CSS-only motifs (cloud, delivery loop, shield, and network fabric) create the requested background theme without reducing readability. The labels stack into a clean two-column mobile layout. Focus-visual and drag/reset data remain preserved but are intentionally not shown in this composition. Editor mode retains in-place label editing, portrait upload, and selected-text Bold action. The work passes 82 tests plus the production build. GitHub feature commit `2e1024d` (`feat: refine floating specialty motion`) contains the visual implementation; it awaits user review before any merge.
