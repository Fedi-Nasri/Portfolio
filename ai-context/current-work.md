...
**Motion and Home-composition prototype branch:** `feature/public-motion-prototype` was created from `main` locally and on GitHub. It adds a staged hero entrance and one-time IntersectionObserver section reveals, scoped under `.public-motion` so `/edit` is unchanged. The branch now also uses a lighter four-role selector: compact visual tiles with direct readable labels, arranged in a balanced two-by-two grid. The portrait is smaller and has a soft lower transparency fade, without changing Fedi’s source image. Editor mode retains in-place text edits, portrait/focus-visual uploads, focus-card dragging, Reset card layout, and now reliably captures selected text offsets for its existing Bold toolbar action; the safe `**bold**` token renders through the shared public `RichText` component. The work honors `prefers-reduced-motion`, uses an observer fallback that leaves content visible, passes 83 tests plus the production build, and awaits user review before any merge.
