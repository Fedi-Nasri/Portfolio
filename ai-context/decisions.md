...
| D-017 | Treat `main` as stable development and `deployment_versel` as the Vercel-connected deployment branch. | Superseded | Replaced by D-022 after the user selected `master` for both future feature work and Vercel Production. Preserve this entry as historical branch-policy evidence. |
| D-018 | Mirror Vercel and API-bridge documentation in `main` while keeping deployed implementation on `deployment_versel`. | Superseded | Replaced by D-022. `master` now contains the active local and Vercel-ready implementation. |
| D-019 | Use `deployment_versel` as Vercel’s Production Branch. | Superseded | Vercel Production Environment Branch Tracking moved to `master` on 2026-08-24 after fresh explicit user confirmation. `deployment_versel` remains the unchanged historical release/rollback reference. |
| D-020 | Use one active development branch and optional local-only Docker Compose. | Superseded | Replaced by D-022. Docker Compose remains local-only, but `master` replaces `main` as the single active branch. |
...
| D-022 | Use `master` as the sole active development and Vercel Production branch. | Active, user-approved | On 2026-08-24 the user approved master-first development and confirmed the live Vercel setting change. Vercel Production redeployed `master` commit `732c0ac` and reached Ready. Future features start from `master`; production pushes require explicit approval. Preserve `deployment_versel` at `e448599` as rollback evidence and do not maintain it as a second active project. |
