  ...
  return (
    <div ref={portfolioRef} className={`reference-portfolio public-motion${dimMode ? " dim-mode" : ""}`}>
      ...
      <main className="public-section-order">
        ...
        <section id="skills" className="ref-section ref-skills role-toolbox-section" style={sectionStyle("skills")}><div className="toolbox-field-header"><SectionTitle eyebrow={content.skillsSection.eyebrow}><Multiline value={content.skillsSection.title} /></SectionTitle><p>Role-aligned capabilities for building, securing, and operating dependable cloud services.</p></div><div className="role-toolbox-grid">{content.skills.map((skill, index) => <article className="toolbox-role-card" key={`${skill.role ?? "focus"}-${skill.heading}`}><header><span className="toolbox-card-index">{String(index + 1).padStart(2, "0")}</span><div><p><RichText value={skill.role ?? "Engineering focus"} /></p><h3><RichText value={skill.heading} /></h3></div></header><TagList items={skill.entries} /></article>)}</div></section>
