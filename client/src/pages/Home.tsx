  ...
  return (
    <div ref={portfolioRef} className={`reference-portfolio public-motion${dimMode ? " dim-mode" : ""}`}>
      ...
      <main className="public-section-order">
        ...
        <section id="skills" className="ref-section ref-skills role-toolbox-section" style={sectionStyle("skills")}><div className="toolbox-field-header"><SectionTitle eyebrow={content.skillsSection.eyebrow}><Multiline value={content.skillsSection.title} /></SectionTitle><p>Role-aligned capabilities for building, securing, and operating dependable cloud services.</p></div><div className="role-toolbox-grid">{content.skills.map((skill) => <article className="toolbox-role-card" key={`${skill.role ?? "focus"}-${skill.heading}`}><header><ToolboxRoleIcon role={skill.role} /><div><p><RichText value={skill.role ?? "Engineering focus"} /></p><h3><RichText value={skill.heading} /></h3></div></header><div className="ref-tags toolbox-tool-list">{skill.entries.map((tool) => <span className="toolbox-tool-chip" key={tool}><RichText value={tool} /></span>)}</div></article>)}</div></section>
